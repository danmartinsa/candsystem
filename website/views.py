...
from website.decorators import candidate_required, evaluator_required
...
from website.forms import selectAssignTestForm, QuestionForm, EvaluateForm
from website.models import User, Role, AssignTest, Evaluation, EvaluationScore
...
def home(request):
    if request.user.is_authenticated:
        ...
        if request.user.is_interviwer:
            ...
            data = Evaluation.objects.filter(evaluator=request.user)
            return render(request, 'website/evaluation.html', {'data': data})
...
class TestView(FormView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        email = form.cleaned_data['candidate']
        category = Category.objects.get(id=form.cleaned_data['category'])
        role = Role.objects.get(id=form.cleaned_data['role'])
        quiz = create_quiz(plang=category, position=role,
                           user_email=email, interviewer=self.request.user)
        ...
        if quiz is not None:
            ...
            requestor_email_subject = "Questionnaire has been sent out to %s" % (
                email)
            requestor_body = """
...
class MakeEvaluation(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.evaluation = get_object_or_404(
            Evaluation, url=self.kwargs['evaluation_id'])
        ...
        return super(MakeEvaluation, self).dispatch(request, *args, **kwargs)

    def get_context_data(self, **kwargs):

        context = super(MakeEvaluation, self).get_context_data(**kwargs)
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.logged_in_user:
            self.sitting = Sitting.objects.get(
                user_id=self.request.user.id, quiz_id=self.quiz.id)
    ...
    def form_valid(self, form, **kwargs):
        ...
        for question in question_list:
            ...
            if question.__class__ is Essay_Question:
                is_correct = ""
                user_answers.append(
                    (str(question.id), question.content, guess))
            else:
                flag_blank = False
                ...
                if guess != '':
                    ...
                    user_answers.append(
                        (str(question.id), question.content, is_correct))
                else:
                    flag_blank = True
                    is_correct = False
